...
function createCacheKey(segment, ctx, opts) {
  ...
  const mandatoryIdPart = `${ctx.req.getMethod()}:${ctx.req.getUrl()}`;
  let id = mandatoryIdPart;

  if (_.get(opts, 'varyOn')) {
    const headers = ctx.req.getHeaders();
    const varyOn = [];

    opts.varyOn.forEach((key) => {
      if (headers[key]) {
        return varyOn.push(`${key}=${headers[key]}`);
      }
      return varyOn.push(`${key}=`);
    });

    id = `${mandatoryIdPart}:${varyOn.join(',')}`;
  }
