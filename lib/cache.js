...
async function getFromCache(cache, segment, ctx, opts) {
  ...
  logger.log(`[read] Cache key creation: [${new Date() - cacheKeyStart}ms]`);
  ...
  try {
    ...
    logger.log(`[read] pending: [${new Date() - pendingStart}ms]`);
    const duration = new Date() - startTime;
    logger.log(`[read] overall: [${duration}ms]`);
...
async function storeInCache(cache, segment, ctx, body, ttl, opts) {
  const cacheKeyStart = new Date();
  const key = createCacheKey(segment, ctx, opts);
  logger.log(`[write] Cache key creation: [${new Date() - cacheKeyStart}ms]`);
  ...
  try {
    const pendingStart = new Date();
    await pending;
    logger.log(`[write] cache.set: [${new Date() - pendingStart}ms]`);
    const duration = new Date() - startTime;
    logger.log(`[write] overall: [${duration}ms]`);
