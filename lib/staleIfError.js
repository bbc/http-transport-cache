...
module.exports = (cache, opts) => {
  return async (ctx, next) => {
    ...
    try {
      await next();
      if (ctx.res.statusCode >= 400) throw new HttpError();
    } catch (err) {
      try {
        cached = await getFromCache(cache, SEGMENT, ctx.req.getUrl(), opts);
      } catch (cacheErr) {
    ...
    if (cacheControl[STALE_IF_ERROR] && !ctx.res.fromCache) {
      ...
      return await storeInCache(cache, SEGMENT, ctx.req.getUrl(), ctx.res.toJSON(), ttl);
    }
  };
};
