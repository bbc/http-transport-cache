...
const startCacheConnection = require('./startCacheConnection').startCacheConnection;
const isCacheReady = require('./startCacheConnection').isCacheReady;
  ...
  return async (ctx, next) => {
    if (!cache.isReady()) {
      try {
        ...
        const useConnectionCircuitBreaker = _.get(opts, 'useConnectionCircuitBreaker');
        await startCacheConnection({ cache, feedName: ctx.feedName, timeout, useConnectionCircuitBreaker, circuitBreaker });
      } catch (err) {
