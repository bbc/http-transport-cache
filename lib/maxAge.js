...
const startCacheConnection = require('./startCacheConnection');
  ...
  return async (ctx, next) => {
    if (!cache.isReady()) {
      try {
        ...
        await startCacheConnection({ cache, feedName: ctx.feedName, timeout });
      } catch (err) {
        if (opts.onCacheConnectionError) opts.onCacheConnectionError(err);
